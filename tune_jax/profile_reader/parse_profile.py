...
def parse_profile_from_bytes(profile_bytes: bytes) -> xplane_pb2.XSpace:  # ty: ignore[unresolved-attribute]
    p = xplane_pb2.XSpace()  # ty: ignore[unresolved-attribute]
    p.ParseFromString(profile_bytes)
    return p


def find_device_plane_ids(p: xplane_pb2.XSpace, device_str: str) -> list[int]:  # ty: ignore[unresolved-attribute]
