...
def _insert_event(event_tree: EventScopeNode, parsed_event: ParsedEvent, raw_event, keys):
  """Insert an event in a named scope tree creating nodes if necessary."""
  if len(keys) == 0:
    return
  child_name = keys[0] if len(keys) > 1 else f"{keys[0]}-{parsed_event.hlo_op}"
  if len(keys) == 1:
    start = raw_event.offset_ps / 1e12
    end = (raw_event.offset_ps + raw_event.duration_ps) / 1e12
    duration = raw_event.duration_ps / 1e12
    event_tree.children[child_name] = EventScopeNode(parsed_event.name, parsed_event, start, end, duration)
  else:
    event_tree.children.setdefault(child_name, EventScopeNode(child_name))
    _insert_event(event_tree.children[child_name], parsed_event, raw_event, keys[1:])


def _combine_times(event_tree: EventScopeNode):
  ...
  all_children_times = [_combine_times(child) for child in event_tree.children.values()]
...
def get_events_from_plane(p: xplane_pb2.XSpace, plane_idx: int) -> dict[str, list[EventScopeNode]]:
  ...
  for line in plane.lines:
    prev_root_key, running_event = None, EventScopeNode()  # running event is a top level named-scope/jit-function
    for _, event in enumerate(line.events):
      ...
      if prev_root_key is not None and prev_root_key != keys[0]:
        # when the top level name changes, we need to aggregate time and save the complete event
        _combine_times(running_event)
      ...
      _insert_event(running_event, parsed_event, event, keys)
      prev_root_key = keys[0]
