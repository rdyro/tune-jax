...
def tune(
    ...
    def _get_best_hyperparams(args, kws):
        ...
        if len(args) == 0 or all(x is None or jax.core.is_concrete(x) for x in jax.tree.leaves(args)):
            ...
            _maybe_typeof = lambda x: x if not isinstance(x, jax.Array) else jax.typeof(x)
            args_val = jax.tree.map(lambda x, s: _get_random_value(_maybe_typeof(x), s), args, shardings)
        ...
        if len(kws) == 0 or all(v is None or jax.core.is_concrete(v) for v in kws.values()):
            ...
            kws_val = jax.tree.map(lambda x: _get_random_value(_maybe_typeof(x)), kws)
        ...
        try:
            ...
            def _timing_closure(repeats: int):
                ...
                for i, hs in hs:
                    ...
                    _time_fn(functools.partial(lambda fn: fn(*args_val, **kws_val), fns[i]), repeat=repeats, number=1)
            ...
            for i, hs in hs_pbar:
                hs = dict(zip(hyperparams_norm.keys(), hs))
                t_mean, t_std = _time_fn(functools.partial(lambda fn: fn(*args_val, **kws_val), fns[i]), repeat=10)
                results[i] = TimingResult(hs, t_mean, t_std)
...
def test_main():
    ...
    print(tuned_mha_jit.timing_results)  # to get access to timing results  # ty: ignore[unresolved-attribute]
